...
var CheckoutComponent = (function () {
    ...
    CheckoutComponent.prototype.onMonthChange = function () {
        //this.expiryMonth = jQuery('#expiryMonth').text();
        this.expiryMonth = $("#expiryMonth").find("option:selected").text();
    };
    CheckoutComponent.prototype.onYearChange = function () {
        this.expiryYear = jQuery('#expiryYear').val();
    };
    ...
    CheckoutComponent.prototype.getToken = function () {
        ...
        if (this.cardNumber == undefined || this.cardNumber == "" || this.cardNumber.trim().length == 0) {
            jQuery('#cardNoInvalid').show();
            flag = false;
        }
        if (this.cvc == undefined || this.cvc == "" || this.cvc.trim().length == 0) {
            jQuery('#cvcNoInvalid').show();
            flag = false;
        }
        if (this.expiryMonth == undefined || this.expiryMonth == "" || this.expiryMonth.trim().length == 0 || this.expiryMonth == "00") {
            jQuery('#monthInvalid').show();
            flag = false;
        }
        if (this.expiryYear == undefined || this.expiryYear == "" || this.expiryYear.trim().length == 0 || this.expiryYear == "0000") {
            jQuery('#yearInvalid').show();
            flag = false;
        }
        if (flag == false) {
            ...
            _this._zone.run(function () {
                if (status === 200) {
                    ...
                    _this._paymentService.charge(_this.Email, response.id, _this.TotalPrice * 100).subscribe(function (data) {
                        _this.creditCardNumber = _this.cardNumber;
                        _this.creditCardNumber = "************" + _this.creditCardNumber.substring(_this.creditCardNumber.length - 4, _this.creditCardNumber.length);
                        _this.creditCardType = response.card.brand;
                        // alert(this.creditCardNumber+ "      "+ this.creditCardType);
                        console.log(data);
                    });
                }
                else {
